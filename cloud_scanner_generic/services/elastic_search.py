...
class ElasticSearch(TableStorage):
    ...
    def __init__(self, config: ElasticSearchConfig):
        self._es_instance = Elasticsearch(config.access_url, http_auth=('elastic', config.access_key))
    ...
    def write(self, resource):
        ...
        self._es_instance.index(index=location, doc_type=self._doc_type, id=identifier, body=entry)
    ...
    def query(self, partition_key, row_key):
        ...
        '''
        task = self._es_instance.get(index=partition_key, doc_type=self._doc_type, id=row_key)
        return task
    ...
    def delete(self, partition_key, row_key):
        ...
        :return: None
        '''
        self._es_instance.delete(index=partition_key,doc_type=self._doc_type, id=row_key)
